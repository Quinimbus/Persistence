...
public class Migrations {
    ...
    private static record EntityTypeMigrationEntry(
            EntityTypeMigration migration, List<String> path, EntityType entityType) {}

    public static void upgradeSchema(PersistenceSchemaStorage storage, Function<String, Schema> schemaFinder)
            throws PersistenceException {
        var schemaMeta = storage.getSchemaMetadata();
        var schema = schemaFinder.apply(schemaMeta.id());
        log.fine(() -> "[upgradeSchema] Upgrading schema %s from version %d to version %d"
                .formatted(schema.id(), schemaMeta.version(), schema.version()));
        var migrationsPerVersionAndType = schema.entityTypes().values().stream()
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.groupingBy(EntityTypeMigrationEntry::entityType, Collectors.toSet())));
    ...
    private static Stream<EntityTypeMigrationEntry> toMigrationEntries(EntityType type) {
        var directMigrations = type.migrations().stream().map(m -> new EntityTypeMigrationEntry(m, List.of(), type));
        var embeddedMigrations =
                type.properties().stream().flatMap(p -> toMigrationEntries(type, p.name(), p.type(), List.of()));
        return Stream.concat(directMigrations, embeddedMigrations);
    }

    private static Stream<EntityTypeMigrationEntry> toMigrationEntries(
            EntityType parentType, String name, EntityTypePropertyType type, List<String> path) {
        if (type instanceof EmbeddedPropertyType embedded) {
            var myPath = new ArrayList<>(path);
            myPath.add(name);
            var directMigrations =
                    embedded.migrations().stream().map(m -> new EntityTypeMigrationEntry(m, myPath, parentType));
    ...
    private static void checkMigrations(
            ...
            Schema schema,
            long version,
            Map<EntityType, Set<EntityTypeMigrationEntry>> migrations)
            throws PersistenceException {
        if (migrations == null || migrations.isEmpty()) {
            log.fine(() ->
                    "[checkMigrations] no migrations needed for schema %s version %d".formatted(schema.id(), version));
            return;
        }
        log.fine(() -> "[checkMigrations] checking for migrations in schema %s for version %d"
                .formatted(schema.id(), version));
        for (var migrationEntry : migrations.entrySet()) {
            checkMigrations(
                    storage, schemaMetadata, schema, version, migrationEntry.getKey(), migrationEntry.getValue());
        }
    }
    ...
    private static void checkMigrations(
            ...
            Set<EntityTypeMigrationEntry> migrations)
            throws PersistenceException {
        if (migrations == null || migrations.isEmpty()) {
            log.fine(() -> "[checkMigrations] no migrations needed for schema %s version %d entity type %s"
    ...
    private static void checkMigration(
            ...
            long version,
            EntityType entityType,
            EntityTypeMigrationEntry migration)
            throws PersistenceException {
        log.fine(() -> "[checkMigration] checking for migration %s in schema %s for version %d entity type %s"
                .formatted(migration.migration().name(), schema.id(), version, entityType));
        if (schemaMetadata.entityTypeMigrationRuns().stream()
                .anyMatch(mr -> mr.entityType().equals(entityType.id())
                        && mr.schemaVersion().equals(version)
                        && mr.identifier().equals(migration.migration().name()))) {
