...
public class FilterFactory {
    ...
    public static record DefaultPropertyFilter(String property, PropertyFilter.Operator operator, Object value)
    ...
    public static <R extends Record> Set<? extends PropertyFilter> fromRecord(R record) throws PersistenceException {
        try {
            return ThrowingStream.of(
                            Arrays.stream(record.getClass().getRecordComponents()), ReflectiveOperationException.class)
                    .map(rc -> new DefaultPropertyFilter(
                            rc.getName(),
                            PropertyFilter.Operator.EQUALS,
                            rc.getAccessor().invoke(record)))
                    .collect(Collectors.toSet());
        } catch (ReflectiveOperationException ex) {
            throw new PersistenceException(
                    "Failed to analyze the filter record type "
