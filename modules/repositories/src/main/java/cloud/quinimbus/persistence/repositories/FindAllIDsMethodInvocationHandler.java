...
public class FindAllIDsMethodInvocationHandler extends RepositoryMethodInvocationHandler {
    ...
    public FindAllIDsMethodInvocationHandler(Class<?> iface, Method m, PersistenceContext ctx)
        ...
        if (getEntityType().owningEntity().isEmpty()) {
            if (m.getParameterCount() != 0) {
                throw new InvalidRepositoryDefinitionException(
                        "The findAllIDs method should only have zero parameters");
            }
        } else {
            if (m.getParameterCount() != 1) {
                throw new InvalidRepositoryDefinitionException(
                        "The findAllIDs method should have one parameter for weak entities");
            }
        }
        ...
        if (List.class.equals(returnType)) {
            ...
            throw new InvalidRepositoryDefinitionException(
                    "Unknown return type for a findAllIDs method: %s".formatted(returnType.getName()));
        }
    }
    ...
    public Object invoke(Object proxy, Object[] args) throws Throwable {
        if (getEntityType().owningEntity().isEmpty()) {
            return this.getSchemaStorage().findAllIDs(this.getEntityType()).collect(Collectors.toList());
        } else {
